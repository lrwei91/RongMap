...
export function applyLocationRules(candidate, context) {
  ...
  if (candidate.category === 'spot' && candidate.confidence === 'low') {
    return {
      action: 'needs_confirmation',
      ruleDecision: 'spot_low_confidence_confirmation',
      message: '景点类低置信地点需要确认后再保存'
    };
  }

  if (candidate.category === 'food' && candidate.confidence === 'high') {
    return {
      action: 'auto_save',
      ruleDecision: 'food_high_confidence_auto_save',
