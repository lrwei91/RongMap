...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (supabase) {
      const linkResult = await supabase.from('share_links').select('id,space_id,label,scope,trip_id,revoked_at,spaces(name)').eq('token_hash', hashToken(token)).is('revoked_at', null).maybeSingle();
