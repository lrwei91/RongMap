...
async function getServiceIdentity() {
  ...
  const { data, error } = await supabase.from('space_members').select('user_id,profiles!space_members_user_id_fkey(name,email)').eq('space_id', spaceId).eq('role', 'admin').eq('status', 'active').limit(1);
