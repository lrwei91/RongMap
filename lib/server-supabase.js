...
async function getRequestIdentity(req) {
  ...
  if (memberships[0].status === 'invited') {
    const invitedSpaceId = userData.user.user_metadata?.invited_space_id;
    if (invitedSpaceId && invitedSpaceId === memberships[0].space_id) {
      ...
      const error = new Error('邀请尚未完成，请从最新邀请邮件进入');
      error.status = 403;
      throw error;
    }
  }
