...
async function bootstrap(identity) {
  ...
  const [spaceResult, locationResult, trashResult, tagResult, memberResult, activityResult, linkResult, tripResult] = await Promise.all([
    ...
    supabase.from('share_links').select('id,label,scope,trip_id,created_at,revoked_at,last_accessed_at').eq('space_id', identity.spaceId).order('created_at', { ascending: false }),
    supabase.from('trips').select('id,space_id,name,description,start_date,status,version,created_by,updated_by,created_at,updated_at,trip_days(id,trip_items(id))').eq('space_id', identity.spaceId).is('deleted_at', null).order('updated_at', { ascending: false }).limit(500)
  ]);
