...
const { getDistanceInMeters, hasFiniteCoordinates, isFiniteCoordinateValue } = require('./location-coordinates');
const { normalizeText } = require('./location-record');

const REMOTE_TEXT_TIMEOUT_MS = 8000;
const REMOTE_TEXT_MAX_BYTES = 512 * 1024;
const REMOTE_TEXT_ALLOWED_HOSTS = ['douyin.com', 'iesdouyin.com'];

function assertAllowedRemoteUrl(value) {
  const parsed = new URL(value);
  const hostname = parsed.hostname.toLowerCase().replace(/\.$/, '');
  if (parsed.protocol !== 'https:' || !REMOTE_TEXT_ALLOWED_HOSTS.some((domain) => hostname === domain || hostname.endsWith(`.${domain}`))) {
    throw new Error('URL 来源不受支持');
  }
  if (parsed.username || parsed.password || parsed.port) {
    throw new Error('URL 格式不受支持');
  }
  return parsed;
}

const CATEGORY_TYPE_RULES = {
  cafe_bar: ['咖啡', '茶艺', '酒吧', '精酿', '小酒馆', 'bistro', 'bar'],
  food: ['餐饮', '甜品', '小吃', '中餐', '西餐', '快餐', '蛋糕', '饮品'],
  spot: ['风景', '景点', '公园', '休闲', '娱乐', '博物馆', '寺庙', '古迹', '度假', '游乐', '文化'],
};

const CATEGORY_TEXT_RULES = {
  cafe_bar: ['咖啡', '咖啡店', '咖啡馆', '酒吧', '精酿', '小酒馆', 'bistro'],
  food: ['火锅', '烤肉', '小吃', '奶茶', '甜品', '餐厅', '饭店', '酒楼', '面馆', '烧烤'],
...
function buildCandidateFromPoi(poi, context = {}) {
  ...
  return {
    ...
    longitude,
    category,
    sourceId: normalizeOptionalText(poi.id) || null,
...
function fetchRemoteText(url, redirectCount = 0) {
  return new Promise((resolve, reject) => {
    ...
    let parsedUrl;
    try {
      parsedUrl = assertAllowedRemoteUrl(url);
    } catch (error) {
      reject(error);
      return;
    }
    ...
    const request = https.request(parsedUrl, {
      ...
      if (!response.statusCode || response.statusCode < 200 || response.statusCode >= 300) {
        response.resume();
        reject(new Error(`远程页面返回 HTTP ${response.statusCode || 0}`));
        return;
      }
      ...
      response.on('data', (chunk) => {
        size += chunk.length;
        if (size > REMOTE_TEXT_MAX_BYTES) {
          request.destroy(new Error('远程页面超过大小限制'));
          return;
        }
        body += chunk;
      });
    ...
    request.setTimeout(REMOTE_TEXT_TIMEOUT_MS, () => request.destroy(new Error('远程页面请求超时')));
    request.end();
  });
}
...
function applyLocationRules(candidate, context) {
  ...
  if (candidate.category === 'food' || candidate.category === 'spot' || candidate.category === 'cafe_bar') {
    return {
      action: 'auto_save',
